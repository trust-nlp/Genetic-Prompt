...
def load_attributes(attr_name = "", model = "", dataset = "", method = "", classes = None):
    ...
    # if attr_name in ["subtopics_filter",  "keywords", "similar", "brands", "product_name", 'product_name_filter', \
    #     'resource',  'resource_filter', 'experience_filter', 'scenario', "scenario_filter", "technique"]: #'experience',
        assert classes is not None 
        return_dict = {} 
        for c in classes:
            return_dict[c] = []
            with open(f"../datasets/{dataset}/{method}/{attr_name}/{c}.jsonl", 'r') as f:
                for lines in f:
        ...
        #print(f"../datasets/{dataset}/{method}/{model}/{attr_name}/{attr_name}.txt")
        with open(f"../datasets/{dataset}/{method}/{attr_name}/{attr_name}.txt", 'r') as f:
            for lines in f:
...
if __name__ == "__main__":
    x=  load_attributes(attr_name = "style", model = "gpt-3.5-turbo", dataset = "nyt", method = "attrprompt", classes = ["dance"])
